...
class Agent():
    ...
    def error_mail(self, location, exception):
        try:
            ...
            email.simple_mail(self.config,[self.email_address], subject, message)
        except Exception as e:
    ...
    def order(self, account_name, tradingsymbol, strategy_name, trade_type, isMarket, price, orderid):
        try:
            query_string = 'insert into '+self.tables['order']+' values("'+self.module_name+'", now(), "'+account_name+'", "'+strategy_name+'", "'+tradingsymbol+'", "'+trade_type+'", '+str(isMarket)+', '+str(price)+', "'+orderid+'");'
            self.connection = self.create_connector()
            self.connection.execute(query_string)
        except Exception as e:
